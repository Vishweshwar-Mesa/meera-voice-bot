...
import { sendMessage, sendChatAction, downloadTelegramFile } from "../lib/telegram.js";
...
const WELCOME_TEXT =
  "Hi! Send me a text note or voice note and I'll turn it into a ready-to-post draft in your voice.";
...
export default async function handler(req, res) {
  ...
  // there's nothing for us to do with it (e.g. a photo with no caption).
  if (!chatId) {
    res.status(200).end();
    return;
  }
  ...
  try {
    if (voice) {
      ...
      const base64Audio = await withRetry(() => downloadTelegramFile(voice.file_id));
      const transcript = await withRetry(() => transcribeAudio(base64Audio, voice.mime_type));
      console.log(`Transcribed voice note from chat ${chatId}: "${transcript.slice(0, 80)}"`);
      await processNote(chatId, transcript);
    } else if (!text) {
      await sendMessage(chatId, "Send me a text note or voice note and I'll draft a post from it.");
    } else if (text === "/start" || text === "/help") {
      await sendMessage(chatId, WELCOME_TEXT);
    } else {
      await processNote(chatId, text);
    }
  } catch (err) {
    console.error("Failed to handle Telegram update:", err);
    try {
      await sendMessage(chatId, "Sorry, something went wrong processing that. Please try again.");
    } catch (sendErr) {
...
async function processNote(chatId, note) {
  const junkRejection = checkNote(note);
  if (junkRejection) {
...
async function handleNote(chatId, note) {
  ...
  const { score, reason, passed } = await withRetry(() =>
    scoreNote(note, {
      newsItem,
  ...
  const { text: draft, usedNews } = await withRetry(() => generateDraft(note, newsItem));
